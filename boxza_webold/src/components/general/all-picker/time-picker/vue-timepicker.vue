<template>
  <span class="time-picker">
    <input class="display-time" :id="id" v-model="timeData" placeholder="00:00" @click.stop="toggleDropdown" type="text" readonly />
    <span class="clear-btn" v-if="!hideClearButton" v-show="!showDropdown" @click.stop="clearTime">&times;</span>
    <div class="time-picker-overlay" v-if="showDropdown" @click.stop="closeDropdown"></div>
    <div class="dropdown" v-show="showDropdown">
      <div class="select-list">
        <ul class="hours">
          <li class="hint" v-text="hourType"></li>
          <li v-for="(hr, index) in hours" v-text="hr" :key="index" :class="{active: hour === hr}" @click.stop="select('hour', hr)"></li>
        </ul>
        <ul class="minutes">
          <li class="hint" v-text="minuteType"></li>
          <li v-for="(m, index) in minutes" v-text="m" :key="index" :class="{active: minute === m, 'disabled': disabledMinutes(m)}" @click.stop="!disabledMinutes(m) ? select('minute', m) : ''"></li>
        </ul>
        <ul class="seconds" v-if="secondType">
          <li class="hint" v-text="secondType"></li>
          <li v-for="(s, index) in seconds" v-text="s" :key="index" :class="{active: second === s}" @click.stop="select('second', s)"></li>
        </ul>
        <ul class="apms" v-if="apmType">
          <li class="hint" v-text="apmType"></li>
          <li v-for="(a, index) in apms" v-text="a" :key="index" :class="{active: apm === a}" @click.stop="select('apm', a)"></li>
        </ul>
      </div>
    </div>
  </span>
...
<script>
  ...
  export default {
    ...
    computed: {
      // disabledMinutes: function (min) { return this.dataDisabledMin.findIndex(dismin => dismin === min) === -1 }
    },
    watch: {
      'format': 'renderFormat',
    ...
    methods: {
      disabledMinutes: function (min) {
        return this.dataDisabledMin.findIndex(dismin => dismin === min) !== -1
      },
      defineVueTimepicker: function (val, formatTime) {
        if (val) {
          if (formatTime === 'HH:mm') {
            ...
            this.minute = moment(val, 'hh:mm').format('mm')
            this.apmType = 'A'
            if (moment(val, 'hh:mm').format('A') === 'หลังเที่ยง') {
              this.apm = 'PM'
            } else {
              this.apm = 'AM'
            }
          }
          let formatString = String(formatTime)
          if (this.hour) {
            formatString = formatString.replace(new RegExp('HH', 'g'), this.hour)
          }
          if (this.minute) {
            formatString = formatString.replace(new RegExp('mm', 'g'), this.minute)
          }
          if (this.apm && this.apmType) {
            formatString = formatString.replace(new RegExp('hh', 'g'), this.hour)
            formatString = formatString.replace(new RegExp('A', 'g'), this.apm)
          }
          this.timeData = formatString
      ...
      displayTime: function () {
        ...
        vm.timeData = formatString
        switch (this.format) {
          case 'HH:mm':
            if (this.hour && this.minute) {
              vm.$emit('timeData', this.timeData)
              vm.showDropdown = false
            }
            break
          case 'hh:mm A':
            if (this.hour && this.minute && this.apm) {
              vm.$emit('timeData', this.timeData)
              vm.showDropdown = false
            }
            break
      ...
      disabledMinVuePicker: function (arrMin) {
        const vm = this
        vm.dataDisabledMin = arrMin
      },
      clearTime () {
        this.hour = ''
        this.minute = ''
...
<style>
  ...
  .time-picker .dropdown ul li.disabled,
  .time-picker .dropdown ul li.disabled:hover {
  ...
  .display-time::-webkit-input-placeholder {
