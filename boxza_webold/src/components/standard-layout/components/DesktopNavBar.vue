<template>
  <div id="desktopNavBar">
    ...
    />
    <div class="boxza-bg" style="position: absolute;">
      <svg
  ...
  <!-- <v-toolbar flat color="transparent" class="nav-new">
      <v-img max-width="100px" :src="require('~/assets/images/icon/logo.png')" :lazy-src="require('~/assets/images/icon/logo.png')" />
      <div class="sub-menu-box is-hidden-mobile" v-for="(menu, index) in menuList" :key="index">
        <div class="sub-menu">
          <span class="menu-text"
                @click="redirect(menu.path)"
                :class="{'active-menu': activeMenu(menu.pathName, $route)}">{{ menu.menuName }}
          </span>
        </div>
      </div>
      <v-spacer />
    </v-toolbar> -->
  <!-- <v-toolbar flat color="transparent" class="toolbar-box">
      <v-btn class="button-register" v-if="!isLogin" @click="dologin($event)">
        <v-btn class="button-register" v-if="!isLogin" @click="isLogin = !isLogin">
        <span class="button-text">เข้าสู่ระบบ</span>
      </v-btn>
    </v-toolbar> -->
</template>
...
#desktopNavBar {
  ...
  .sub-menu-box {
    ...
    .sub-menu {
      ...
      line-height: 1.2;
      text-align: center;
      -webkit-transform: matrix(0.75765864332604, 0, 0, 0.75903614457831, 0, 0);
